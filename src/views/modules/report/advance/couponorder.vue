    ...
    <div class="show-box">
      ...
      <div class="tb-box">
        ...
        >
          ...
          <el-table-column prop="extraCharge" label="金贝" align="right">
            <template slot-scope="scope">
...
<script>
import { FormatAllDate, getTimeByType, validator,formatDate } from "@/utils/timeUtils";
import { formatDateTime,IsInArray } from "@/utils";
export default {
  data() {
    return {
      title: "查询窗口（点击展开）",
  ...
  methods: {
    ...
    async query() {
      this.q.startTime = formatDate(this.value[0]) + ' 08:00:00';
      this.q.endTime = formatDate(this.value[1]) + ' 23:30:00';
    ...
    download() {
      ...
      this.q.startTime = formatDate(this.value[0]);
      this.q.endTime = formatDate(this.value[1]);
      this.url =
    ...
    getSummaries(param) {
      ...
      const sums = [];
      const pArr = ['orderMoney','realUseMoney','backMoney','profitClose','profitCloseSub','overDueMoney','chargeOpen','extraCharge'];
      columns.forEach((column, index) => {
        ...
        if (IsInArray(pArr,column.property)) {//指定列合计
