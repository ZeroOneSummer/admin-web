<template>
  <div class="page-container">
    <div class="show-box" v-show="!switchForm">
      ...
      <div class="tag-box">
        <ul>
          <li>
            {{keyword}}总额：
            <span>{{count.totalBounty}}</span>
          </li>
          <li>
            可用{{keyword}}总额：
            <span>{{count.usableBounty}}</span>
          </li>
          <li>
            已消费{{keyword}}总额：
            <span>{{count.consumeBounty}}</span>
          </li>
          <li>
            已过期{{keyword}}总额：
            <span>{{count.pastBounty}}</span>
          </li>
          <li>
            15日内即将过期{{keyword}}：
            <span>{{count.LastTwoBounty}}</span>
          </li>
        </ul>
      </div>
      <div class="tb-box">
    ...
    <div v-if="showDetails" class="child-page">
      <dealerdetail @goback="goback(value)" :queryTime="value" :id="detailId"></dealerdetail>
    </div>
  </div>
</template>
<script>
...
import dealerdetail from "./children/dealerdetail";
export default {
  data() {
    return {
      labelPosition: "right",
  ...
  methods: {
    async getCount() {
      let r = await this._fetch("//hxbounty/count", { method: "GET" });
      if (r.code === 0) {
        this.count = r.data;
      } else {
        console.log(r.msg);
    ...
    async query() {
      ...
      let r = await this._fetch(
        "/hxbounty/dealerCount?limit=50&page=1" + queryParams,
        {
          method: "GET"
        }
      );
    ...
    download() {
      ...
      this.q.endTime = formatDate(this.value[1]) + " 23:59:59";
      this.url =
        "/hxbounty/download?type=dealer&token=" +
        token +
        "&startTime=" +
        this.q.startTime +
        "&endTime=" +
    ...
    async handleSizeChange(val) {
      let r = await this._fetch(
        "/hxbounty/dealerCount?limit=" + val + "&page=" + this.page.currentPage,
        { method: "GET" }
      );
      this.tableData = r.page.list;
