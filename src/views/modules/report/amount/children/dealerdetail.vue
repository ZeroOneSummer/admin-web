  ...
  methods: {
    async query() {
      ...
      let r = await this._fetch(
        "/bountyDailySettlement/dealerList/"+this.id+"?limit=10&page=1" + queryParams,
        {
          method: "GET"
        }
      );
    ...
    download() {
      ...
      this.q.endTime = formatDate(this.value[1]) + " 23:59:59";
      this.url =
        "/bountyDailySettlement/download/dealerList/" +
        this.id +
        "?token=" +
        token +
        "&startTime=" +
